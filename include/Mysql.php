...
class Mysql
{
    ...
    var $cache_data_dir = 'data/temp/query_caches/';
    var $root_path      = '';
    ...
    function __construct($param = array(), $pconnect = 0, $quiet = 0)
    {
        $this->Mysql($param, $pconnect, $quiet);
    }

    function Mysql($param = array(), $pconnect = 0, $quiet = 0)
    {
		if (defined('DB_NAME') && defined('PREFIX'))
		{
			$this->dbname = DB_NAME;
        ...
        {
            $charset = strtolower(str_replace('-', '', RK_CHARSET));
        }
        ...
        if ($quiet)
        {
            $this->connect($param[0], $param[1], $param[2], $param[3], $param[4], $pconnect, $quiet);
        }
        else
        {
            $this->settings = array(
    ...
    function table($str)
    {
        return '`' . $this->dbname . '`.`' . $this->dbprefix . $str . '`';
    }
